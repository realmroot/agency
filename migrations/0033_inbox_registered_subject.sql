-- Persist the remote-confirmed Inbox subject and the in-flight replacement
-- target. Rebuild triggers so D1 constraints stay identical to Drizzle while
-- foreign-key enforcement is enabled.
CREATE TABLE `__backup_0033_session_routes` AS SELECT * FROM `session_routes`;--> statement-breakpoint
CREATE TABLE `__backup_0033_http_trigger_pending_runs` AS SELECT * FROM `http_trigger_pending_runs`;--> statement-breakpoint
CREATE TABLE `__backup_0033_trigger_runs` AS SELECT * FROM `trigger_runs`;--> statement-breakpoint
...
INSERT INTO `__new_triggers` (
	...
	`inbox_provisioning_state`, `inbox_provisioning_error`, `metadata`,
	`created_by_user_id`, `archived_at`, `created_at`, `updated_at`
)
SELECT
	t.`id`, t.`organization_id`, t.`project_id`, t.`agent_id`, t.`environment_id`,
	t.`trigger_type`, t.`http_concurrency_mode`, t.`runtime`, t.`name`, t.`prompt_template`,
	t.`env`, t.`env_from`, t.`volumes`, t.`volume_mounts`, t.`interval_seconds`,
	t.`window_seconds`, t.`enabled`, t.`next_due_at`, t.`last_dispatched_at`, t.`last_run_id`,
	t.`inbox_subscription_id`, t.`inbox_callback_token_hash`, t.`inbox_callback_token_ciphertext`,
	t.`inbox_subscription_etag`, NULL,
	CASE WHEN t.`trigger_type` = 'inbox' THEN json_extract(a.`identity_snapshot`, '$.subject') ELSE NULL END,
	CASE WHEN t.`trigger_type` = 'inbox' AND t.`inbox_provisioning_state` = 'active' THEN 'pending' ELSE t.`inbox_provisioning_state` END,
	t.`inbox_provisioning_error`, t.`metadata`, t.`created_by_user_id`, t.`archived_at`,
	t.`created_at`, t.`updated_at`
FROM `triggers` t
LEFT JOIN `agents` a ON a.`id` = t.`agent_id`;--> statement-breakpoint
...
INSERT INTO `trigger_runs` SELECT * FROM `__backup_0033_trigger_runs`;--> statement-breakpoint
DROP TABLE `__backup_0033_trigger_runs`;--> statement-breakpoint
...
INSERT INTO `http_trigger_pending_runs` SELECT * FROM `__backup_0033_http_trigger_pending_runs`;--> statement-breakpoint
DROP TABLE `__backup_0033_http_trigger_pending_runs`;--> statement-breakpoint
...
INSERT INTO `session_routes` SELECT * FROM `__backup_0033_session_routes`;--> statement-breakpoint
DROP TABLE `__backup_0033_session_routes`;--> statement-breakpoint
