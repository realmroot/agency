    ...
    vaults: {
      ...
      insertCredentialWithVersion: async (
        ...
        const version: CredentialVersion = {
          ...
          spec: {
            ...
            version: 1,
            provider: 'enbor',
            secretRef: `enbor://vaults/${versionInput.vaultId}/credentials/${versionInput.credentialId}/versions/${versionInput.id}`,
      ...
      insertVersionRotation: async (input: { id: string; credentialId: string; vaultId: string; version: number }) => ({
        ...
        spec: {
          ...
          provider: 'enbor',
          secretRef: `enbor://vaults/${input.vaultId}/credentials/${input.credentialId}/versions/${input.id}`,
          referenceName: 'IDENTITY_STATE',
          hasSecret: true,
          metadata: {},
        },
...
function descriptor(identityId: string): IdentityDescriptor {
  return {
    ...
    username: 'reviewer',
    runtime: 'codex',
    credentialRef: `enbor://vaults/vault_${identityId}/credentials/cred_${identityId}`,
  }
}
