...
- The verified Realmroot `client_id` selects the permitted credential mode. Direct API clients use Realmroot Bearer or DPoP credentials; the browser uses an AMA HttpOnly cookie backed by a verified Realmroot access token. A credential presented through the wrong mode is rejected without fallback.
