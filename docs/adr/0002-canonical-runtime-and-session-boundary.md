...
- All agent products run as Session-selected runtimes behind the same Enbor
...
- Self-hosted runners own host execution while Enbor retains Session lifecycle,
  dispatch, authorization, and event ownership.
- Browsers, SDKs, and command-line helpers use Enbor Session endpoints rather than
...
- Every runtime adapter translates provider, model, tool, workspace, policy,
  lifecycle, usage, and error activity into the canonical Enbor Session event
...
- Enbor does not define an incompatible runtime SDK or protocol. The Cloudflare
