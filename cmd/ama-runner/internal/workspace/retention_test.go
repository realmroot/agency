...
func TestCleanupStalePreservesDurableSessionHistory(t *testing.T) {
	...
	if _, err := eventLog.Append(ama.JSON{
		"type":    "assistant.message",
		"payload": ama.JSON{"text": "retained transcript"},
	}); err != nil {
		t.Fatalf("append canonical event: %v", err)
	}
	if _, err := session.AppendProviderEvent(sessionRoot, "codex", ama.JSON{
		"type": "turn.completed",
	}); err != nil {
...
func TestCleanupStaleRetriesFailedWorktreeCleanupWithoutDiscardingSessionEvidence(t *testing.T) {
	...
	if _, err := eventLog.Append(ama.JSON{"type": "assistant.message", "payload": ama.JSON{"text": "retry history"}}); err != nil {
		t.Fatalf("append canonical event: %v", err)
	}
	if _, err := session.AppendProviderEvent(root, "codex", ama.JSON{"type": "turn.completed"}); err != nil {
		t.Fatalf("append provider event: %v", err)
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			createLog: func(t *testing.T, root string) {
				...
				if _, err := store.Append(ama.JSON{"type": "session.completed", "payload": ama.JSON{}}); err != nil {
					t.Fatalf("append canonical event: %v", err)
				}
			},
		...
		},
		{
			...
			createLog: func(t *testing.T, root string) {
				if _, err := session.AppendProviderEvent(root, "claude-code", ama.JSON{"type": "result"}); err != nil {
					t.Fatalf("append provider event: %v", err)
				}
			},
