...
func TestEventLogAppendReadAllAndReopen(t *testing.T) {
	...
	for i, typ := range []string{"a", "b", "c"} {
		event, err := store.Append(enbor.JSON{"type": typ, "payload": enbor.JSON{"i": i}})
		if err != nil {
	...
	if enborEvent := events[0].EnborEvent(); enborEvent["type"] != "a" || enborEvent["payload"] == nil {
		t.Fatalf("unexpected Enbor event projection %#v", enborEvent)
	}
	...
	event, err := reopened.Append(enbor.JSON{"type": "d", "payload": enbor.JSON{}})
	if err != nil {
...
func TestProviderEventLogAppendReadAndValidate(t *testing.T) {
	...
	dir := t.TempDir()
	first, err := AppendProviderEvent(dir, "codex", enbor.JSON{"type": "item.completed", "item": enbor.JSON{"id": "item_1"}})
	if err != nil {
		t.Fatalf("append provider event: %v", err)
	}
	second, err := AppendProviderEvent(dir, "codex", enbor.JSON{"type": "turn.completed"})
	if err != nil {
	...
	if _, err := AppendProviderEvent(dir, "", enbor.JSON{"type": "item.completed"}); err == nil || !strings.Contains(err.Error(), "runtime") {
		t.Fatalf("expected missing runtime error, got %v", err)
	}
	if _, err := AppendProviderEvent(dir, "codex", enbor.JSON{"event": "missing type"}); err == nil || !strings.Contains(err.Error(), "missing type") {
...
func TestAppendProviderEventReturnsDirectoryError(t *testing.T) {
	...
	if _, err := AppendProviderEvent(filepath.Join(parentFile, "session"), "codex", enbor.JSON{"type": "turn.completed"}); err == nil {
...
func TestEventLogAppendCoordinatesSequenceAcrossOpenStores(t *testing.T) {
	...
	if event, err := first.Append(enbor.JSON{"type": "message.completed", "payload": enbor.JSON{"writer": "first"}}); err != nil || event.Sequence != 1 {
		t.Fatalf("first append sequence = %d err=%v, want 1", event.Sequence, err)
	}
	if event, err := second.Append(enbor.JSON{"type": "message.completed", "payload": enbor.JSON{"writer": "second"}}); err != nil || event.Sequence != 2 {
		t.Fatalf("second append sequence = %d err=%v, want 2", event.Sequence, err)
	}
	if event, err := first.Append(enbor.JSON{"type": "message.completed", "payload": enbor.JSON{"writer": "first"}}); err != nil || event.Sequence != 3 {
...
func TestEventLogReadsEventsLargerThanOldScannerLimit(t *testing.T) {
	...
	if _, err := store.Append(enbor.JSON{"type": "message.completed", "payload": enbor.JSON{"text": largeText}}); err != nil {
...
func TestEventLogAppendValidatesTypeAndNormalizesPayload(t *testing.T) {
	...
	if _, err := store.Append(enbor.JSON{"payload": enbor.JSON{}}); err == nil || !strings.Contains(err.Error(), "missing type") {
		t.Fatalf("expected missing type error, got %v", err)
	}
	if _, err := store.Append(enbor.JSON{"type": ""}); err == nil || !strings.Contains(err.Error(), "missing type") {
		t.Fatalf("expected empty type error, got %v", err)
	}
	event, err := store.Append(enbor.JSON{"type": "tool.result", "payload": map[string]any{"toolCallId": "call_1"}})
	if err != nil {
	...
	event, err = store.Append(enbor.JSON{"type": "message.completed", "payload": "ignored"})
	if err != nil {
...
func TestEventLogReportsInvalidAndUnwritableLogs(t *testing.T) {
	...
	if _, err := store.Append(enbor.JSON{"type": "message.completed"}); err == nil {
		t.Fatal("expected append under file path to fail")
	}
	if _, err := (&EventLog{path: filepath.Join(t.TempDir(), "events.jsonl"), sessionID: "session_1"}).Append(enbor.JSON{"bad": func() {}}); err == nil {
