...
func EnsureCompatibleConfig(config *enborsdk.PublicConfig) error {
	if config == nil {
...
func RunnerOidcSettingsFromConfig(config *enborsdk.PublicConfig, fallbackResource string) (RunnerOidcSettings, error) {
	if err := EnsureCompatibleConfig(config); err != nil {
		return RunnerOidcSettings{}, err
	}
	if config.Auth.Oidc == nil {
		return RunnerOidcSettings{}, fmt.Errorf("Enbor config is missing OIDC settings")
	}
	oidc := config.Auth.Oidc
	if strings.TrimSpace(oidc.Issuer) == "" {
		return RunnerOidcSettings{}, fmt.Errorf("Enbor config is missing OIDC issuer")
	}
	if oidc.Runner == nil || strings.TrimSpace(oidc.Runner.ClientId) == "" {
		return RunnerOidcSettings{}, fmt.Errorf("Enbor config is missing runner OIDC client")
	}
	scopes := strings.Join(oidc.Runner.Scopes, " ")
	if strings.TrimSpace(scopes) == "" {
		return RunnerOidcSettings{}, fmt.Errorf("Enbor config is missing runner OIDC scopes")
	}
