...
import { ApiError, api } from './enborrpc'
import { jsonArg, paramQueryArg, queryArg, queryOptions, rpcRequest } from './enborrpc/core'
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, _init?: RequestInit) => {
      ...
      if (url === '/api/v1/configz') {
        return new Response(
          JSON.stringify({
            version: 1,
            service: { name: 'Enbor', origin: window.location.origin },
            auth: {
              oidc: {
