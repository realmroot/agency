Feature: Projects
  A project is the tenant scope for every Enbor resource. External products such as
  Downstream systems own their own workflow but use Enbor as the lower-level agent,
  environment, session, runner, and event substrate through the OpenAPI SDK —
  storing only standard Enbor fields and keeping product mappings in their own store.
  ...
  Scenario: External product manages standard Enbor resources through the SDK
    Given an external product owns its workflow identifiers and product state
    When the product creates or updates Enbor agent definitions, environments, and resources through the OpenAPI SDK
    Then Enbor stores only standard Enbor resource fields
    And Enbor does not store product-specific external references as first-class fields
    And the product keeps any mapping between product records and Enbor ids in its own storage
    And Enbor does not require the product to expose board, task, review, or PR concepts

  @projects/external-session @e2e
  Scenario: External product starts work by creating an Enbor session
    Given an external product has selected a standard Enbor agent definition
    And the external product has selected a standard Enbor environment
    And the external product has selected standard Enbor resource references
    When the external product creates an Enbor session through the OpenAPI SDK
    Then Enbor snapshots the selected agent and environment
    And Enbor validates the session runtime, provider, and model before runtime work starts
    And Enbor returns a stable session id, status, status reason, runtime, and event endpoint
    And the external product can store the returned Enbor ids in its own product records
    And the external product can render progress from Enbor session status and canonical events

  @projects/external-control @e2e
  Scenario: External product controls a running session only through Enbor endpoints
    Given an external product created an Enbor session
    When the external product sends a follow-up message, close request, or resume request
    Then Enbor routes the command to the selected runtime or owning self-hosted runner
    And Enbor records the command result as canonical session events
